        ...
        if (user.userCardId) {
          stripe.customers.create({
            ...
            .then(newCard => {
              ...
              newCardDetails.map(obj => {
                // console.log(newCard.sources.data[0])
                // return
                let new_data =  newCard.sources.data
                //eslint-disable-line
                new_data.map(obj_new =>{
                  console.log(obj_new.fingerprint)
                  console.log("here")
                  console.log(obj.fingerprint)
                  if (obj_new.fingerprint === obj.fingerprint) {
                    checkUser = true;
              ...
              // } else
               if (paymentDetails.saveCard) {
                ...
                newCardDetails.push(newCard.sources.data[0]);
          ...
          stripe.customers
            ...
            .then(source => {
              ...
              const newCardDetails = user.cardDetails;
              newCardDetails.push(source.sources.data[0]);
              User.findOneAndUpdateAsync(
                { _id: user._id },
                {
                  $set: {
...
function cardPayment(tripObj) {
  ...
  //   amount: 1000,
  //   currency: 'usd',
  //   customer: 'cus_H4UDbJiqcxBCE8',
  //   source: 'tok_1GWNATHHPLhdheLYIbxeiDhT',
  // }));
  // });
  // return true;
...
// 		"email":"virtual34@mailinator.com",
// 	"tripAmt":"1000",
// 	"customer":"cus_H4WCgRDxa89USk"
// }
  ...
  return new Promise((resolve, reject) => {
    getStripeKey().then(key => {
      ...
      .then(foundUser => {
          ...
          const user = foundUser;
          console.log(user.cardDetails[0].customer)
          stripe.charges
            .create({
              amount: 1000,
              currency: "usd",
              customer: user.cardDetails[user.cardDetails.length - 1].customer
              // source: tripObj.source
            ...
            .then(charge => {
              ...
              //   amount: Number(tripObj.tripAmt) / 100,
...
function earning(req,res,next){
  Transaction
  .find({
    userIdTo: req.body.driverId
 },
 )
.then(updateWalletObj => {
  var sum = 0;

  updateWalletObj.forEach((obj)=>{
    sum = sum+obj.amount
  })
