...
function addWorkImage(req,res,next) {
  ...
  form.parse(req, (err, fields, files) => {
    console.log("next step")
      ...
      const imgpath1 = files.image1;
      const imgpath2 = files.image2;
      const imgpath3 = files.image3;
      
      if(fields.price ===undefined || fields.info_photographer===undefined || fields.camera_lenses===undefined || fields.address === undefined ){
        const returnObj = {
          success: false,
      ...
      cloudinary.v2.uploader.upload(imgpath1.path, (error, results1) => {
        console.log(results1)
        if (error) {
          console.log(error, "error upload");
        }
        if (results1) {
          // workPhoto1 = results.url
          User.findOneAndUpdateAsync(
            { _id: req.user._id }, //eslint-disable-line
            { $set: { workPhoto1:results1.url } },
            { new: true }
          ).then(
            cloudinary.v2.uploader.upload(imgpath2.path, (error, results2) => {
              if (error) {
                console.log(error, "error upload");
              }
              if (results2) {
                User.findOneAndUpdateAsync(
                  { _id: req.user._id }, //eslint-disable-line
                  { $set: { workPhoto2:results2.url } },
                  { new: true }
                ).then(
                  cloudinary.v2.uploader.upload(imgpath3.path, (error, results3) => {
                    if (error) {
                      console.log(error, "error upload");
                    }
                    if (results3) {
                      User.findOneAndUpdateAsync(
                        { _id: req.user._id }, //eslint-disable-line
                        { $set: { workPhoto3:results3.url ,info_photographer:fields.info_photographer ,camera_lenses:fields.camera_lenses,price:fields.price,city:fields.city,state:fields.state,country:fields.country ,address:fields.address} },
                        { new: true }
                      ).then(
                        savedUser => {
                              const returnObj = {
