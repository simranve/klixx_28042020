...
import startTripHandler from './start-trip';
...
function requestTripHandler(socket) {
  ...
  /////////////for requesting particular photographer
  
/////////////for requesting particular photographer
socket.on("requestPhotographer", payload => {
   
  const quantum = 10;
  const photographer_id = payload.photographer_id;
  const riderID = payload.rider._id;

 console.log(payload)
  createTripRequestObjAsync(payload,photographer_id)
    .then(requestPhotographerData => {
      // console.log(nearByDriversDoc, 'nearby user');
      // for (let i = 0; i < requestPhotographerData.length - 1; i++) {
      //   if (!checkSocketConnection(requestPhotographerData[i]._id)) {
      //     requestPhotographerData = removeDriverFromList(requestPhotographerData, i);
      //   }
      // }
      // console.log(payload)
      // console.log("requestPhotographerData")
      // console.log(requestPhotographerData)
      socket.emit("requestTrip",{"requestTripData":requestPhotographerData})
...
///photographer response in the form of accept and reject
socket.on("photographerResponse", payload => {
   
  const tripId = payload.tripId;
  const tripRequestStatus = payload.tripRequestStatus;
  const tripRequestObj = {tripRequestStatus:payload.tripRequestStatus,latitudeDelta:payload.latitudeDelta,longitudeDelta:payload.longitudeDelta,pickUpAddress:payload.pickUpAddress,srcLoc:payload.srcLoc,destLoc:payload.destLoc,destAddress:payload.destAddress,tripIssue:payload.tripIssue}
  console.log("simran checkingggggggggg")
  console.log(payload)
  // const TripUpdate = TripRequest.findByIdAndUpdate(tripId,{$set:{tripRequestStatus:tripRequestStatus}});
  // console.log(TripUpdate)
  const TripUpdate = TripRequest.findOneAndUpdateAsync(
    { _id: payload.tripId },
    { $set: tripRequestObj },
    { new: true }
  ) 
  socket.emit("requestTrip",{"requestTripData":tripRequestStatus})
...
/////////////
  ...
  socket.on("requestDriverResponse", tripRequestObj => {
    ...
    // socket.emit("startTrip",tripRequest =>{
    //   console.log("simran")
    // })
    // promObj[driverId].resolve(tripRequest); // or resolve promise
    // socket.emit("requestTrip",{"requestTripData":tripRequest})
    ...
    tripObj
        ...
        UserSchema.updateAsync(
          ...
          .then(() => {
            fetchReturnObj(savedTrip).then(returnObj => {
              SendNotification(riderID, "Photographer has accepeted the request");
      ...
      socket.emit("requestDriverResponse",{"requestDriverResponse":tripObj})
  });
  ...
  function sendRequestAsync(driver, timeout, rider, def) {
    ...
    createTripRequestObjAsync(rider, driver)
      .then(tripRequestObj => {
        ...
        SendNotification(driver._id, "New Request");
  ...
  function createTripRequestObjAsync(payload, driver) {
    return new Promise(resolve => {
      ...
      const latDelta = payload.rider.latitudeDelta;
      const lonDelta = payload.rider.longitudeDelta;
      const paymentMode = payload.tripRequest.paymentMode;
      const driverID = driver;
      const bookTime = payload.tripRequest.bookTime;
      ...
      tripRequestObj.saveAsync()
      .then(savedTripRequest => {

        console.log("this is simran")
        UserSchema.findOneAsync({ _id: payload.rider._id })
          .then(foundUser => {
            if (foundUser !== null) {
              console.log("this is simran here")
              sendEmail(driver, tripRequestObj, "requestTripForPhotoGrapher")

              // sendSms(driver,"Hi, "+foundUser.fname+ " has requested for photography at "+payload.tripRequest.destAddress)
      
              SendNotification(driver,"Hi, "+foundUser.fname+ " has requested for photography at "+payload.tripRequest.destAddress)
            }
          })
        ...
        // UserSchema.findOneAsync({ _id: driverID })
        // .then(userDoc =>{

        //   const accountSid = 'ACce5efac448dcb11e1269cec0ded3cd45';
        //   const authToken = 'd5b6f10c0d3f8383373091c845ba1957';
        ...
        // }).then(function () {
          ...
          // .then(() => {
            ...
            return resolve(savedTripRequest)
            // );
          // })
    //       });
    //     }).error(function (e) {
  ...
  //     tripRequestObj
  //       .saveAsync()
  //       .then(function (savedTripRequest) {
